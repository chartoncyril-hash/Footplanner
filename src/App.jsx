...
import { EventResponsePage } from './components/EventResponsePage';
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const [{ data: profile }, { data: family }, { data: allLicencies }, { data: clubMember }] = await Promise.all([
        ...
        supabaseClient.from('club_members').select('id, club_owner_id, status').eq('user_id', user.id).eq('status', 'active').maybeSingle(),
      ]);
      ...
      if (savedSpace === 'licencie' && licencie) {
        ...
        if (fpId) {
          const { error: flErr } = await supabaseClient.from('family_licencies')
            .upsert({ family_user_id: user.id, licencie_id: licencie.id, relation: 'self', club_owner_id: licencie.owner_id },
...
{item.id==='planning' && <CalendarDays size={16} />}
          ...
          <div style={{ padding:'12px 8px', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
            ...
            <button onClick={() => { setHubView('account'); if (hubMobile) setHubMenuOpen(false); }} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', borderRadius:8, border:'none', background: hubView==='account' ? 'rgba(100,116,139,0.15)' : 'transparent', color: hubView==='account' ? '#94a3b8' : '#64748b', cursor:'pointer', fontSize:13, fontWeight:500, textAlign:'left', width:'100%', fontFamily:'inherit', marginBottom:2, borderLeft: hubView==='account' ? '2px solid #64748b' : '2px solid transparent' }}>
              <span style={{ fontSize:15 }}>👤</span> Mon compte
            </button>
            <button onClick={() => { setHubView('settings'); if (hubMobile) setHubMenuOpen(false); }} style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px', borderRadius:8, border:'none', background: hubView==='settings' ? 'rgba(100,116,139,0.15)' : 'transparent', color: hubView==='settings' ? '#94a3b8' : '#64748b', cursor:'pointer', fontSize:13, fontWeight:500, textAlign:'left', width:'100%', fontFamily:'inherit', marginBottom:2, borderLeft: hubView==='settings' ? '2px solid #64748b' : '2px solid transparent' }}>
